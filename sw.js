...
const CACHE='iufit-v1031-watch-credit-onboarding';
const FILES=['./','./index.html','./manifest.json','./icon-192.png','./icon-512.png','./iufit-modern-theme.css','./food-db.js','./resultcard.html','./iufit-iu-mate.css','./iufit-iu-mate.js','./native-wear-bridge.js','./iufit-icons.js','./iufit-workout-icons.js','./iufit-master-icons.js','./food-icon-normalizer.js','./food-icon-matcher.js','./food-icon-menu-mapping.js','./open.html','./badge-appstore.svg','./badge-googleplay.svg'];
...
self.addEventListener('push',function(e){
 ...
 var title=d.title||'IUFIT \ud83d\udcaa';
 var opts={ body:(d.body||''), icon:(d.icon||'./icon-192.png'), badge:'./icon-192.png', tag:(d.tag||'iufit'), renotify:true, data:{url:(d.url||'/')} };
...
self.addEventListener('notificationclick',function(e){
 ...
 e.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(function(cl){
  for(var i=0;i<cl.length;i++){ if(cl[i].url.indexOf(self.location.origin)>=0){ try{cl[i].focus();}catch(_){}; try{ if(target&&target!=='/'&&'navigate' in cl[i]) cl[i].navigate(target); }catch(_){}; return; } }
