...
function buildEquipReply(firstEq,message){
  ...
  if(wantPlan){
    ...
    plan.days.forEach(function(d){ body+='\n'+'🗓️ '+d.label+' — '+d.focus+'\n'+d.exercises.map(function(ex,i){return (i+1)+'. '+_equipExName(ex)+' — '+ex.sets+' × '+ex.reps;}).join('\n')+'\n'; });
  ...
  var msg=(k.desc||'')+'\n\n'+L('กล้ามเนื้อหลัก: ','Trains: ')+(k.mus||[]).join(' · ')+'\n'+L('รูปแบบการเคลื่อนไหว: ','Patterns: ')+(k.pat||[]).map(function(p){return MOVEMENT_PATTERNS[p]||p;}).join(', ');
  if(exs.length){ msg+='\n\n'+L('ท่าที่เล่นได้:','Exercises you can do:')+'\n'+exs.slice(0,7).map(function(ex,i){return (i+1)+'. '+_equipExName(ex)+' ('+ex.sets+'×'+ex.reps+')';}).join('\n'); var cues=(exs[0]&&exs[0].cue)||[];if(cues.length)msg+='\n\n'+L('เคล็ดลับ: ','Tip: ')+cues.join(' · '); }
  if(k.safe)msg+='\n\n⚠️ '+k.safe;
...
function buildLibraryRecommend(opts){opts=opts||{};
  ...
  return { title:L('เมนู'+_m+'ที่แนะนำ','Recommended menus'),
    message:L('จากคลังเมนู '+menuList().length.toLocaleString()+' รายการ ผมเลือกเมนู'+_m+'ที่เข้ากับเป้าหมาย "'+_g+'" ให้ครับ','From '+menuList().length.toLocaleString()+' menus, here are ones that fit your goal "'+goalLabel()+'" and remaining calories:'),
    recipes:recs.slice(0,3),
    actions:[{label:L('สร้างจากของที่มี','From my ingredients'),action:'open_ingredient_picker'},{label:L('เปิดหน้าอาหาร','Open Food'),action:'go_food'}] };
}
