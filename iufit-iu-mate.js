...
function botAvatar(){ return '<img src="assets/iu-mate/06-iu-mate-face-happy.webp" alt="" style="width:100%;height:100%;object-fit:contain;display:block">';var _ba='<svg viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:block"><circle cx="28" cy="28" r="25.9" fill="none" stroke="#D8EBFF" stroke-width="1.22"/><circle cx="28" cy="28" r="24.5" fill="#EEF6FF" stroke="#7EB9FF" stroke-width="1.75"/><g transform="scale(0.875)"><circle cx="32" cy="11.6" r="4" fill="#0A84FF"/><circle cx="13.8" cy="29.3" r="3.8" fill="#0A84FF"/><circle cx="50.2" cy="29.3" r="3.8" fill="#0A84FF"/><rect x="17.8" y="19.6" width="28.4" height="17.4" rx="8" fill="#071E4A"/><path d="M23.7 26.6C23.7 25.1 24.9 23.9 26.4 23.9C27.9 23.9 29.1 25.1 29.1 26.6" stroke="#0A84FF" stroke-width="2.2" stroke-linecap="round"/><path d="M34.9 26.6C34.9 25.1 36.1 23.9 37.6 23.9C39.1 23.9 40.3 25.1 40.3 26.6" stroke="#0A84FF" stroke-width="2.2" stroke-linecap="round"/><path d="M28.4 31C29.2 31.8 30.4 32.3 32 32.3C33.6 32.3 34.8 31.8 35.6 31" stroke="#0A84FF" stroke-width="2.2" stroke-linecap="round"/><path d="M32 50.2L25.7 46.45C24 45.44 22.95 43.63 22.95 41.66C22.95 38.53 25.5 35.98 28.64 35.98C30.16 35.98 31.54 36.52 32.55 37.52C33.56 36.52 34.94 35.98 36.46 35.98C39.6 35.98 42.15 38.53 42.15 41.66C42.15 43.63 41.1 45.44 39.4 46.45L32 50.2Z" fill="#0A84FF"/><path d="M27.85 42.5H30.05L31.3 39.9L33.1 44.4L34.55 41.55H37.25" stroke="#FFFFFF" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></g></svg>'; }
...
var INTENT_KW = {
  ...
  cuisine_menu:['เมนูญี่ปุ่น','อาหารญี่ปุ่น','เมนูจีน','อาหารจีน','เมนูฝรั่ง','อาหารฝรั่ง','อาหารตะวันตก','เมนู 7-11','เมนูสะดวกซื้อ','เซเว่น','7-11','เมนูร้านสะดวกซื้อ','japanese','chinese','western','convenience','korean','italian','mexican','indian food'],
  budget_menu:['เมนูประหยัด','เมนูถูก','งบน้อย','เมนูงบ','ประหยัดเงิน','ราคาถูก','เมนูราคาประหยัด','cheap menu','budget meal','on a budget'],
  food_recommend:['กินอะไรดี','แนะนำเมนู','เมนูแนะนำ','แคลต่ำ','โปรตีนสูง','ลดไขมัน','หิว','อยากกิน','กินไรดี','เมนูอะไรดี','ขอเมนู','ไม่รู้จะกินอะไร','เบื่ออาหาร','มื้อนี้กินอะไร','ของว่าง','what to eat','recommend menu','low cal','high protein','suggest food','hungry','meal idea','snack idea','what should i eat','food suggestion','recommend a meal','recommend food'],
  food_search:['หาอาหาร','ค้นหาเมนู','มีเมนู','search food','find menu','looking for a menu','search for food'],
  result_summary:['ผลลัพธ์','น้ำหนักตอนนี้','น้ำหนักลดไป','ดูน้ำหนัก','ลดไปกี่','ความคืบหน้า','กราฟ','รอบเอว','result','progress','my result','waist','my weight','how much did i lose','results','stats','my progress','my results'],
  workout_recommend:['ออกกำลัง','ท่าฝึก','เล่นอะไร','เวท','คาร์ดิโอ','ดัมเบล','เล่นไรดี','ท่าอะไรดี','เบื่อท่าเดิม','ขอท่า','ลดพุง','ลดต้นขา','บริหาร','เล่นกล้าม','workout','exercise','train','cardio','weight training','move idea','what to train','training ideas','what workout should i do','recommend a workout','workout ideas'],
  ...
  make_plan:['จัดแผนให้','ช่วยวางแผน','วางแผนลด','วางแผนเพิ่มกล้าม','ทำแผนลด','จัดโปรแกรม','จัดแผนอาหาร','วางแผนให้ฉัน','make a plan','help me plan','plan for me','build a plan','meal plan for me','diet plan','nutrition plan'],
  calc_plan:['คำนวณแคล','คำนวณโปรตีน','ตั้งแคล','ควรกินกี่แคล','แคลเท่าไร','โปรตีนเท่าไหร่','โปรตีนกี่กรัม','กี่กรัม','มาโคร','macro','tdee','bmr','bmi','calorie target','คำนวณมาโคร','ควรตั้งแคล','calculate my calories','how many calories should i eat','calculate calories','calculate my macros'],
  ...
  coach_progress:['สรุปลูกเทรน','วิเคราะห์ลูกเทรน','ความคืบหน้าลูกเทรน','สรุปลูกเทรนวันนี้','ใครน่าห่วง','ใครเสี่ยงหลุด','ภาพรวมลูกเทรน','progress client','client risk','who is at risk','clients overview','client summary','how are my clients doing'],
  coach_followup:['ติดตามลูกเทรน','ลูกเทรนที่ต้องติดตาม','ใครยังไม่ส่ง','ใครหาย','คนที่ต้องติดตาม','follow up','who to follow','inactive client','who hasn','clients to follow up'],
  coach_homework_summary:['สรุปการบ้าน','ตรวจการบ้าน','การบ้านค้าง','ส่งการบ้าน','homework','to review','pending homework','review homework'],
  coach_templates:['ข้อความสำเร็จรูป','เทมเพลตข้อความ','ขอข้อความ','ช่วยพิมพ์ข้อความ','พิมพ์ข้อความหาลูกเทรน','ข้อความหาลูกเทรน','message template','templates','draft message','message templates'],
  coach_feedback:['เขียน feedback','feedback','ฟีดแบ็ก','คอมเมนต์ลูกเทรน','ชมลูกเทรน','เขียนคำชม','write feedback','give feedback','write feedback for client'],
  coach_group_summary:['สรุปกลุ่ม','กิจกรรมกลุ่ม','ภารกิจกลุ่ม','กลุ่มไหน','group summary','group activity','mission','group overview'],
  app_help:['ใช้งาน','ทำยังไง','สอน','วิธี','เข้ากลุ่ม','บันทึกอาหาร','how to','how do i','tutorial','guide','what can this app do'],
...
var LEX2C_EN={
  ...
  'place':'PLACE','aplace':'PLACE','findaplace':['PLACE','WORKOUT'],'findplace':['PLACE','WORKOUT'],'workoutspot':['PLACE','WORKOUT'],'placetotrain':['PLACE','WORKOUT'],'wheretotrain':['PLACE','WORKOUT'],'somewheretoworkout':['PLACE','WORKOUT'],'somewheretoexercise':['PLACE','WORKOUT'],'wherecaniworkout':['PLACE','WORKOUT'],'trainnearme':['PLACE','WORKOUT'],
  'add':'LOG','addfood':'LOG','logthis':'LOG','entry':'LOG',
  'howmanycalories':['CALC','CALORIE'],'calculatecalories':['CALC','CALORIE'],
  'myweight':['WEIGHT','RESULT'],'howmuchdidilose':'RESULT','howmuchweighthaveilost':'RESULT','myresults':'RESULT',
  'cut':'GOAL_LOSE','gain':'GOAL_GAIN','bulk':'GOAL_GAIN',
  'starving':'HUNGRY','famished':'HUNGRY',
  'daysperweek':'DAYS','timesaweek':'DAYS',
  'training':'WORKOUT','workingout':'WORKOUT','trainingsession':'WORKOUT',
  'mexican':'CUISINE','indian':'CUISINE',
  'inexpensive':'BUDGET','lowbudget':'BUDGET',
  'member':'CLIENT','members':'CLIENT',
  'construct':'MAKE','regimen':'PLAN','programme':'PLAN',
...
var KNOWLEDGE = [
  ...
  { id:'app_overview', cat:'app_help', kw:['ทำอะไรได้บ้าง','มีหน้าอะไร','ฟีเจอร์','แนะนำฟีเจอร์','สอนใช้งานทั้งหมด','สอนใช้แอป','สอนใช้งานแอป','วิธีใช้แอป','วิธีใช้งานแอป','วิธีใช้','คู่มือ','ภาพรวมแอป','แอปมีอะไร','หน้าไหนทำอะไร','what can this app do','features','app overview','all features','use the app','how to use the app','get started','app tour'],
...
function msgHtml(m){
  ...
  if(m.role==='botText') return '<div class="iu-mate-row-bot"><span class="mini">'+botAvatar()+'</span><div class="iu-mate-bubble bot">'+esc(m._greet?greeting():m.text)+'</div></div>';
  // bot reply object
...
var IUMate = {
  ...
  showPrivacy:function(){
    ...
    if(clr) clr.onclick=function(){ clearHist(); ST.messages=[]; if(ST.isOpen&&hasConsent()){ ST.messages.push({role:'botText',text:greeting(),_greet:1}); ST._skipAnim=true; renderSheet(); } appToast(L('ล้างประวัติแชทแล้ว','Chat history cleared')); wrap.remove(); };
    wrap.querySelector('.no').onclick=function(){ wrap.remove(); };
    wrap.addEventListener('click',function(e){ if(e.target===wrap) wrap.remove(); });
  ...
  _syncNudge:function(){ try{ injectEntryPoints(); }catch(e){} },
